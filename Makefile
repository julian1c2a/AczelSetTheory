...
## Lock a file: make lock FILE=AczelSetTheory/Module.lean
lock:
	@[ -n "$(FILE)" ] || (echo "Usage: make lock FILE=AczelSetTheory/Module.lean" && exit 1)
	@bash git-lock.bash lock $(FILE)

## Unlock a file: make unlock FILE=AczelSetTheory/Module.lean
unlock:
	@[ -n "$(FILE)" ] || (echo "Usage: make unlock FILE=AczelSetTheory/Module.lean" && exit 1)
