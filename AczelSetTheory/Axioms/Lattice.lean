...
theorem union_comm (A B : HFSet) : union A B = union B A :=
  extensionality _ _ fun x => by
    rw [mem_union x A B, mem_union x B A, or_comm]

theorem inter_comm (A B : HFSet) : inter A B = inter B A :=
  extensionality _ _ fun x => by
...
theorem union_assoc (A B C : HFSet) : union (union A B) C = union A (union B C) :=
  extensionality _ _ fun x => by
    rw [mem_union x (union A B) C, mem_union x A B,
        mem_union x A (union B C), mem_union x B C, or_assoc]

theorem inter_assoc (A B C : HFSet) : inter (inter A B) C = inter A (inter B C) :=
  extensionality _ _ fun x => by
...
theorem union_idem (A : HFSet) : union A A = A :=
  extensionality _ _ fun x => by
    rw [mem_union x A A, or_self]

theorem inter_idem (A : HFSet) : inter A A = A :=
  extensionality _ _ fun x => by
...
theorem union_inter_absorb (A B : HFSet) : union A (inter A B) = A :=
  extensionality _ _ fun x => by
    rw [mem_union x A (inter A B), mem_inter A B x]
    exact ⟨fun h => h.elim id And.left, Or.inl⟩

theorem inter_union_absorb (A B : HFSet) : inter A (union A B) = A :=
  extensionality _ _ fun x => by
...
theorem union_inter_distrib (A B C : HFSet) :
...
theorem inter_union_distrib (A B C : HFSet) :
...
theorem setminus_self (A : HFSet) : setminus A A = empty :=
  extensionality _ _ fun x => by
    rw [mem_setminus A A x]
    exact ⟨fun ⟨h, hn⟩ => absurd h hn, fun he => absurd he (not_mem_empty x)⟩

theorem setminus_empty (A : HFSet) : setminus A empty = A :=
  extensionality _ _ fun x => by
    rw [mem_setminus A empty x]
    exact ⟨And.left, fun h => ⟨h, not_mem_empty x⟩⟩

theorem empty_setminus (A : HFSet) : setminus empty A = empty :=
  extensionality _ _ fun x => by
