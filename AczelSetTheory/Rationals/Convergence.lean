...
-- Sucesiones acotadas y convergentes en ℚ₀, y las implicaciones básicas:
...
--   IsBounded       : (ℕ₀ → ℚ₀) → Prop
--   ConvergesTo     : (ℕ₀ → ℚ₀) → ℚ₀ → Prop
--   IsConvergent    : (ℕ₀ → ℚ₀) → Prop
...
theorem isBounded_of_isCauchy {f : ℕ₀ → ℚ₀} (h : IsCauchy f) : IsBounded f := by
...
theorem isCauchy₂_of_convergesTo {f : ℕ₀ → ℚ₀} {L : ℚ₀} (h : ConvergesTo f L) :
    IsCauchy₂ f := by
  intro n m hnm
...
theorem isCauchy_of_convergesTo {f : ℕ₀ → ℚ₀} {L : ℚ₀} (h : ConvergesTo f L) :
    IsCauchy f :=
  (isCauchy_iff_isCauchy₂ f).mpr (isCauchy₂_of_convergesTo h)

theorem isCauchy_of_isConvergent {f : ℕ₀ → ℚ₀} (h : IsConvergent f) : IsCauchy f := by
  obtain ⟨L, hL⟩ := h
...
theorem isBounded_of_convergesTo {f : ℕ₀ → ℚ₀} {L : ℚ₀} (h : ConvergesTo f L) :
    IsBounded f := by
...
theorem isBounded_of_isConvergent {f : ℕ₀ → ℚ₀} (h : IsConvergent f) : IsBounded f := by
  obtain ⟨L, hL⟩ := h
...
`1/2^n` es cero. (Propiedad arquimediana diádica de ℚ₀.) -/
theorem eq_zero_of_le_pow2_all : ∀ {q : ℚ₀}, 0 ≤ q → (∀ n : ℕ₀, q ≤ pow2 n) → q = 0 := by
  intro q
  ...
  | _ p =>
    ...
    have hc0 : (0 : ℤ₀) ≤ p.1 := by
      ...
      rwa [ℤ₀.zero_mul, show ℤ₀.ofNat den1.val = (1 : ℤ₀) from rfl, ℤ₀.mul_one] at h0
    have hc : p.1 = ℤ₀.ofNat (ℤ₀.toNat p.1) := ℤ₀.nonneg_eq_ofNat hc0
    -- ∀ n, (toNat p.1) · 2^n ≤ p.2.val
    have hkey : ∀ n : ℕ₀,
        Peano.Mul.mul (ℤ₀.toNat p.1) (Peano.Pow.pow (σ (σ 𝟘)) n) ≤ p.2.val := by
      intro n
      have hn := (mk_le_mk p.1 (ℤ₀.ofNat (σ 𝟘)) p.2 (pow2_den n)).mp (h n)
      rw [show ℤ₀.ofNat (σ 𝟘) = (1 : ℤ₀) from rfl, ℤ₀.one_mul, hc, ← ℤ₀.ofNat_mul] at hn
      exact ℤ₀.le_ofNat_iff.mp hn
    -- el numerador (en ℕ₀) es cero
    have hnum0 : ℤ₀.toNat p.1 = 𝟘 := by
      by_cases hz : ℤ₀.toNat p.1 = 𝟘
      · exact hz
      · exfalso
        have h1 : (σ 𝟘 : ℕ₀) ≤ ℤ₀.toNat p.1 := by omega₀
        ...
        have hmono : Peano.Order.le₀ (Peano.Pow.pow (σ (σ 𝟘)) (σ p.2.val))
            (Peano.Mul.mul (ℤ₀.toNat p.1) (Peano.Pow.pow (σ (σ 𝟘)) (σ p.2.val))) := by
