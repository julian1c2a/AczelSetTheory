...
-- Lemas de orden para la potencia natural `ℚ₀cls.pow` (definida en Roots.lean).
--
-- API:
--   pow_zero, pow_succ, pow_one, one_pow
--   pow_add        : pow x (m + n) = pow x m * pow x n
--   zero_le_one    : (0 : ℚ₀cls) ≤ 1
...
-- defeq) para que los lemas de anillo de ℚ₀cls (en forma `*`) encajen por `rw`.
--
...
theorem pow_zero (x : ℚ₀cls) : pow x 𝟘 = 1 := rfl

theorem pow_succ (x : ℚ₀cls) (n : ℕ₀) : pow x (σ n) = x * pow x n := rfl

theorem pow_one (x : ℚ₀cls) : pow x 𝟙 = x := by
  show x * pow x 𝟘 = x
  rw [pow_zero, mul_one]


theorem pow_add (x : ℚ₀cls) (m n : ℕ₀) :
...
theorem zero_le_one : (0 : ℚ₀cls) ≤ 1 := ofNat₀_nonneg 𝟙

theorem pow_nonneg {x : ℚ₀cls} (hx : 0 ≤ x) (n : ℕ₀) : 0 ≤ pow x n := by
  induction n with
  | zero => rw [pow_zero]; exact zero_le_one
  | succ k ih => rw [pow_succ]; exact mul_nonneg hx ih

theorem pow_le_pow_left {x y : ℚ₀cls} (hx : 0 ≤ x) (hxy : x ≤ y) (n : ℕ₀) :
    pow x n ≤ pow y n := by
  induction n with
...
theorem one_le_pow {x : ℚ₀cls} (hx : 1 ≤ x) (n : ℕ₀) : 1 ≤ pow x n := by
  induction n with
  | zero => rw [pow_zero]; exact le_refl 1
  | succ k ih =>
    rw [pow_succ]
    calc (1 : ℚ₀cls) = (1 : ℚ₀cls) * 1 := (mul_one (1 : ℚ₀cls)).symm
...
theorem absVal_pow (x : ℚ₀cls) (n : ℕ₀) : absVal (pow x n) = pow (absVal x) n := by
  induction n with
