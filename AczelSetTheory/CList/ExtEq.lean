...
theorem subset_mono
  ...
  (h : evalOp .subset (mk xs) (mk ys) = true) :
    evalOp .subset (mk xs) (mk (y :: ys)) = true
      := by
  induction xs with
  | nil => simp [evalOp]
  | cons z zs ih =>
...
private def bool_and_split {a b : Bool}
  (h : a && b = true) :
    a = true ∧ b = true
      := by
  cases a <;> cases b <;> simp_all

private def bool_or_split {a b : Bool}
  (h : a || b = true) :
    a = true ∨ b = true
      := by
  cases a <;> cases b <;> simp_all

private def bool_and_join {a b : Bool}
  (ha : a = true) (hb : b = true) :
    a && b = true
      := by
  simp [ha, hb]

private def bool_or_join_left {a b : Bool}
  (ha : a = true) :
    a || b = true
      := by
  simp [ha]

private def bool_or_join_right {a b : Bool}
  (hb : b = true) :
    a || b = true
      := by
  simp [hb]
...
theorem subset_cons
  (x : CList) (xs : List CList) (B : CList) :
    subset (mk (x :: xs)) B = (mem x B && subset (mk xs) B)
      := by
...
theorem mem_cons
  (x y : CList) (ys : List CList) :
    mem x (mk (y :: ys)) = (extEq x y || mem x (mk ys))
      := by
