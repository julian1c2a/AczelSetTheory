...
| `GroupTheory/Zassenhaus.lean` | ✅ | `Algebra/Zassenhaus.lean` | **Lema de la Mariposa de Zassenhaus** (2026-06-05): `(H∩K)/[(N∩K)(H∩M)] ≅ N(H∩K)/N(H∩M)` cuando `N ⊴ H` y `M ⊴ K`. Construcciones: `prodSubgroup` (`N·S`), `mem_prodSubgroup_iff`, `prodNKHM` (`(N∩K)(H∩M)`), `prodN_HK` (`N(H∩K)`), `prodN_HM` (`N(H∩M)`), normalidades correspondientes (`inter_N_K_normal_in_inter_H_K`, `inter_H_M_normal_in_inter_H_K`, `prodNKHM_normal`, `prodN_HM_normal_in_prodN_HK`), homomorfismo de Zassenhaus y `zassenhaus_bijection`. 0 sorries. |
...
> **Nota 2026-06-03:** `Sylow I` cerrado.
...
**Última actualización:** 2026-06-05 — FASE A completa (M1–M7); Lema de Zassenhaus cerrado.

1. **Completado (2026-06-05)**: `Zassenhaus.lean` — Lema de la Mariposa ✅. **FASE A cerrada.**
2. **Completado (2026-06-04)**: Sylow II `sylowConjugate` — sorry cerrado vía `p_group_fixed_point`.
...
6. **Baja prioridad** (matemáticas avanzadas que esperan ASet₁):
