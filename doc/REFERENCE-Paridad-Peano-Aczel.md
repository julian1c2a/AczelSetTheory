...
| `PeanoNat/Lattice.lean` | ❌ | — | Peano define `min`, `max`, `min_max`, `max_min` sobre `ℕ₀`. AczelSetTheory carece de `LatticeVN.lean` con `minVN`/`maxVN`; `GcdVN.lean` cubre la retícula de divisibilidad (gcd/lcm) pero **no** la retícula de orden lineal. **Pendiente: `LatticeVN.lean`.** |
...
| `Combinatorics/Group.lean` | ⚠️ | `Algebra/Group.lean` | Peano define `FinGroup` — grupo finito abstracto sobre tipos ordenados. AczelSetTheory tiene `HFGroup` en `Algebra/Group.lean` — equivalente abstracto sobre HFSet. No existe `SymGroupVN.lean` concreto para `Sₙ` sobre VN. **Pendiente: `SymGroupVN.lean`.** |
...
| `Foundation/Initiality.lean` | ❌ | — | Peano prueba `ℕ₀_initial`: existe un único morfismo de Peano desde `ℕ₀` a cualquier `PeanoSystem`. AczelSetTheory verifica que `vN` es morfismo pero no prueba la iniciality formal ni la unicidad del morfismo. **Pendiente: `InitialityVN.lean`.** |
...
1. **Alta prioridad** (prerrequisito para Sylow en Aczel):
   - `LatticeVN.lean` — min/max sobre VN; retícula de orden lineal.
   - `CountingVN.lean` — principios de recuento, necesario para combinatoria.
   - `PermVN.lean` — permutaciones sobre VN/conjuntos finitos.
   - `SignVN.lean` — paridad de permutaciones.

2. **Media prioridad**:
   - `OrbitVN.lean` — órbitas; necesario para ecuación de clases.
   - `ActionVN.lean` — acciones de grupo.
   - `SylowVN.lean` — teoremas de Sylow sobre VN.
   - `InitialityVN.lean` — propiedad inicial de `ℕ₀` como sistema de Peano.

3. **Baja prioridad** (matemáticas avanzadas que esperan ASet₁):
   - Zassenhaus, CRT de grupos, análisis, números reales computables.
